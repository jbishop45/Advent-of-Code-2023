...
for (let i=0; i<seeds.length/2; i++) {
    seed_intervals.push([parseInt(seeds[2*i]),parseInt(seeds[2*i])+parseInt(seeds[2*i+1])-1]);
}
...
// Helper for Maintaining Non-Overlapping Intervals (too lazy, not necessary)
...
for (let m=0; m<maps.length; m++) {
    ...
    console.log('M'+m); // I miss you so much
    // Iterate Over Maps Intervals
    for (let i=0; i<map_intervals.length; i++) {
        x = map_intervals[i][0];
        y = map_intervals[i][1];
        z = map_intervals[i][2];
        // Until Input Interval Buffer Empty
